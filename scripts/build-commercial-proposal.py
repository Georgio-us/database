...
included = [
    ...
    "функции, зафиксированные в техническом задании;",
    "подключение OLX и DIM.RIA при наличии доступного API;",
    "подключение согласованного способа отправки подборок через Telegram;",
    "базовый перенос объектов из согласованного файла;",
...
excluded = [
    ...
    "Telegram-бот или мини-приложение с отдельной логикой, если оно не входит в согласованное техническое задание;",
    "нестандартный парсинг сторонних сайтов;",
    "новые функции, которых нет в согласованном техническом задании;",
    "постоянное сопровождение и дальнейшее развитие базы.",
]
...
add_paragraph(
    doc,
    "При небольшом объёме использования сервер и хранение данных будут стоить ориентировочно $10-$30 в месяц. Фактическая сумма зависит от количества фотографий, документов, пользователей и операций.",
)
...
add_paragraph(
    doc,
    "Для подключения OLX и DIM.RIA нужны рабочие корпоративные аккаунты и доступы к API. Стоимость размещений, лимиты объявлений и модерация определяются самими площадками и не входят в стоимость разработки.",
)
add_paragraph(
    doc,
    "Если нужная функция недоступна через официальный API, возможный вариант реализации согласовывается отдельно до начала интеграции.",
)

doc.add_heading("Передача системы", level=1)
add_paragraph(
    doc,
    "Агентство получает рабочую систему и полный доступ к своим данным. Универсальное ядро и отдельные технические компоненты могут использоваться разработчиком в других проектах. Данные агентства, документы и контакты не передаются и не используются вне системы.",
)

doc.add_heading("Начало работы", level=1)
add_paragraph(
    doc,
    "Для старта необходимо согласовать состав базы, предоставить пример текущей таблицы объектов и проверить доступы к OLX и DIM.RIA. После подготовки технического задания фиксируются итоговая стоимость и календарный план.",
)
